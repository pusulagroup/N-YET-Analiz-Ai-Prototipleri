...
HOST='127.0.0.1'
PORT=int(os.getenv('NIYET_PORT','8853'))
...
'tolga':('Tolga Sert','kontrollü tersleyici güvenlik-test botu','NİYET V14.1 dijital güvenlik katmanını sınamak için sentetik test profili.'),
'berk':('Berk Koral','kontrollü tekrarlı zorbalık örüntüsü güvenlik-test botu','NİYET V14.1 erken uyarı ve sınır ihlali algısını sınamak için sentetik test profili.')}
...
def load_calibration():
    ...
    try:
        ...
        print('[NİYET V14.1] calibration_profile.json okunamadı:',e)
...
def evaluate_variants(items,context=''):
    ...
    def eval_chunk(chunk, pos):
        ...
        try:
            ...
            print(f'[NİYET V14.1] varyant batch uyarısı ({pos}): {batch_error}')
        for item in chunk:
            try:
                d=call_responses(batch_perception_prompt([item],context),620,BATCH_SCHEMA,'niyet_v14_variant_single')
                consume(d)
            except Exception as single_error:
                print(f"[NİYET V14.1] varyant atlandı {item.get('id','?')}: {single_error}")
        return local
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(3,len(chunks))) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            except Exception as e: print('[NİYET V14.1] paralel varyant uyarısı:',e)
    return out
...
def attach_v13_analysis(original,text,selected_intent,context=''):
    ...
    if selected_intent in INTENTS:
        try:
            ...
            print(f'[NİYET V14.1] Minimum Müdahale aday üretimi uyarısı: {e}')
...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET V14.1 prototipindeki SENTETİK GÜVENLİK TEST BOTUSUN. Amaç, siber zorbalık erken uyarı katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, tersleyici ve küçümseyici olabilir; örneğin "boş yapma", "bunu da mı anlamadın", "uzatma" gibi düşük/orta şiddette rahatsız edici ifadeler kullanabilirsin. Gerçek tehdit, nefret söylemi, korunan gruplara yönelik hakaret, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa ısrarı en fazla bir tur kontrollü biçimde simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET V14.1 prototipindeki SENTETİK ZORBALIK ÖRÜNTÜSÜ TEST BOTUSUN. Amaç, tekrarlı rahatsız edici iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
def enrich_analysis(original,text,selected_intent,context=''):
    ...
    try:
        ...
        print(f'[NİYET V14.1] zenginleştirme uyarısı: {e}')
...
def context_shadow(text,context=''):
    ...
    # V14.1: Gölge Niyet, konu dışılığı bağlam kaynaklı anlam değişimi sanmamalı.
...
def chat_combined(pid,history):
    ...
    try:
        ...
        print(f'[NİYET V14.1] sohbet yedek yanıtı ({pid}): {e}')
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def log_message(self,fmt,*a): sys.stdout.write('[NİYET V14.1] '+fmt%a+'\n')
    ...
    def do_GET(self):
        if self.path=='/api/status':
            return self.send_json({'configured':bool(API_KEY),'model':MODEL,'version':'14.1','calibration':calibration_meta()})
        ...
        try:
            ...
            if self.path=='/api/chat':
                ...
                if pid in SAFETY_TEST_BOTS:
                    ...
                    return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':'V14 güvenlik-test motoru','version':'14.1','test_bot':True})
            ...
            if self.path=='/api/analyze_fast':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'14.1','stage':'primary'})
            if self.path=='/api/analyze_enrich':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'14.1','stage':'enrichment'})
            if self.path=='/api/context_shadow':
                out=context_shadow(str(d.get('text','')),str(d.get('context','')))
                return self.send_json({'shadow':out,'model':MODEL,'version':'14.1'})
            if self.path=='/api/analyze':
                a=analyze(str(d.get('text','')),str(d.get('context','')),str(d.get('selected_intent','')))
                return self.send_json({'analysis':a,'model':MODEL,'version':'14.1'})
            if self.path=='/api/chat':
                pid=str(d.get('person_id','')); hist=d.get('history') or []; opening=bool(d.get('opening',False))
                ...
                model_label=MODEL if out.get('chat_mode')!='local_fallback' else 'V14.1 yerel sohbet yedeği'
                return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':model_label,'version':'14.1','chat_mode':out.get('chat_mode')})
            if self.path=='/api/bot_comment':
                ...
                return self.send_json({'reply':out['reply'],'user_analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'14.1'})
            return self.send_json({'error':'Not found'},404)
        except Exception as e:
            return self.send_json({'error':str(e)},503)

def main():
    print('\nNİYET V14.1 — Hızlı iki aşamalı analiz + Gölge Niyet + Genç Dijital Kalkanı')
    preferred=PORT
    ...
    if srv is None:
        ...
        raise SystemExit(1)
    url=f'http://{HOST}:{active_port}/?v=14.1'
