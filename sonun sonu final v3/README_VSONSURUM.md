...
## Summary
NİYET VFINAL2; ifade önerisi erişilebilirliğini, açık hakaret için beklemesiz güvenlik akışını, beş eşit ağırlıklı NİYET skorunu, daha anlaşılır skor ekranını, gelişmiş sohbet yönetimini, görsel destekli AI sohbetini, yeniden paylaşım profil akışını ve daha gerçekçi tartışma örneklerini tek final sürümde birleştirir.
...
- Sürüm etiketi `VFINAL2` olarak güncellendi ve N'SOSYAL × NİYET başlığıyla dikey hizası düzeltildi.
...
ZIP'i **tamamen bir klasöre çıkardıktan sonra** Windows'ta `BASLAT_VFINAL2_WINDOWS.bat` dosyasına çift tıklayın. Tarayıcı otomatik olarak `http://127.0.0.1:10000` adresini açar. Bu başlatıcı için Flask veya Gunicorn kurulması gerekmez; yalnızca Python 3 yeterlidir.

macOS/Linux için `BASLAT_VFINAL2_MAC_LINUX.command` kullanılır.
...
## Final notu
Bu paket yarışma öncesi terminoloji, skor geçerliliği ve servis-dayanıklılığı düzeltmelerini içerir. Beğeni/yeniden paylaşım NİYET skorundan ayrıdır; açık hakaret nefret söylemi olarak etiketlenmez; AI servis kesintisinde yerel ön analiz ve anlaşılır hata mesajı korunur.
