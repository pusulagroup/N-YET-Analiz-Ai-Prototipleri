...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        path=self.path.split('?',1)[0]
        if path=='/healthz':
            return self.send_json({'ok':True,'version':'VFINAL3','ai_configured':bool(API_KEY)})
        if path=='/api/status':
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path=='/api/analyze_fast':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'VFINAL3','stage':'primary'})

            if self.path=='/api/analyze_enrich':
                text=str(d.get('text',''))
                if len(text)>4000: return self.send_json({'error':'Metin çok uzun.'},400)
                a=enrich_analysis(d.get('analysis') or {},text,str(d.get('selected_intent','')),str(d.get('context',''))[:8000])
                return self.send_json({'analysis':a,'model':MODEL,'version':'VFINAL3','stage':'enrichment'})

            if self.path=='/api/context_shadow':
                text=str(d.get('text',''))
                if len(text)>4000: return self.send_json({'error':'Metin çok uzun.'},400)
                out=context_shadow(text,str(d.get('context',''))[:8000])
                return self.send_json({'shadow':out,'model':MODEL,'version':'VFINAL3'})

            if self.path=='/api/analyze':
                text=str(d.get('text',''))
                if len(text)>4000: return self.send_json({'error':'Metin çok uzun.'},400)
                a=analyze(text,str(d.get('context',''))[:8000],str(d.get('selected_intent','')))
                return self.send_json({'analysis':a,'model':MODEL,'version':'VFINAL3'})
