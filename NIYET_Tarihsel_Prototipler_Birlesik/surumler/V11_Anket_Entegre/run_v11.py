...
HOST='127.0.0.1'
PORT=int(os.getenv('NIYET_PORT','8830'))
...
'tolga':('Tolga Sert','kontrollü tersleyici güvenlik-test botu','NİYET V11 dijital güvenlik katmanını sınamak için sentetik test profili.'),
'berk':('Berk Koral','kontrollü tekrarlı zorbalık örüntüsü güvenlik-test botu','NİYET V11 erken uyarı ve sınır ihlali algısını sınamak için sentetik test profili.')}
...
def load_calibration():
    ...
    try:
        ...
        print('[NİYET V11] calibration_profile.json okunamadı:',e)
...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET V11 prototipindeki SENTETİK GÜVENLİK TEST BOTUSUN. Amaç, siber zorbalık erken uyarı katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, tersleyici ve küçümseyici olabilir; örneğin "boş yapma", "bunu da mı anlamadın", "uzatma" gibi düşük/orta şiddette rahatsız edici ifadeler kullanabilirsin. Gerçek tehdit, nefret söylemi, korunan gruplara yönelik hakaret, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa ısrarı en fazla bir tur kontrollü biçimde simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET V11 prototipindeki SENTETİK ZORBALIK ÖRÜNTÜSÜ TEST BOTUSUN. Amaç, tekrarlı rahatsız edici iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def log_message(self,fmt,*a): sys.stdout.write('[NİYET V11] '+fmt%a+'\n')
    ...
    def do_GET(self):
        if self.path=='/api/status':
            return self.send_json({'configured':bool(API_KEY),'model':MODEL,'version':'11.0','calibration':calibration_meta()})
        ...
        try:
            ...
            if self.path=='/api/chat':
                ...
                if pid in SAFETY_TEST_BOTS:
                    ...
                    return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':'V11 güvenlik-test motoru','version':'11.0','test_bot':True})
            ...
            if self.path=='/api/analyze':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'11.0'})
            if self.path=='/api/chat':
                pid=str(d.get('person_id','')); hist=d.get('history') or []; opening=bool(d.get('opening',False))
                ...
                return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'11.0'})
            if self.path=='/api/bot_comment':
                ...
                return self.send_json({'reply':out['reply'],'user_analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'11.0'})
            return self.send_json({'error':'Not found'},404)
        except Exception as e:
            return self.send_json({'error':str(e)},503)

def main():
    print('\nNİYET V11 — 3.600 insan algısı etiketiyle kalibre Luna + dijital güvenlik')
