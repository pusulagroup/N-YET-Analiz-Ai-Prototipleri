#!/usr/bin/env python3
# NİYET V6 local server: built only with Python standard library.
...
def ask_openai(pid,history):
    if not API_KEY: raise RuntimeError('OPENAI_API_KEY tanımlı değil')
    input_items=[{'role':'developer','content':instructions(pid)}]
    for m in history[-12:]:
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path=='/api/status': return self.send_json({'configured':bool(API_KEY),'model':MODEL,'mode':'openai' if API_KEY else 'fallback'})
...
def main():
    global MODEL
    print('\nNİYET V6.1 — OpenAI sohbetli prototip')
