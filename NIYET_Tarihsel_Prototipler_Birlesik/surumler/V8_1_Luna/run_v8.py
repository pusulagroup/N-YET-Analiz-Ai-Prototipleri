...
HOST='127.0.0.1'; PORT=int(os.getenv('NIYET_PORT','8818'))
...
ANALYSIS_SCHEMA={
 'type':'object','properties':{
  ...
  'reason':{'type':'string'}},
 'required':['intent_distribution','has_insult','has_personal_attack','has_dismissive_language','has_contempt','is_unconstructive','has_unexplained_contradiction','escalates_tension','is_ambiguous','has_risky_irony','is_calming','is_polite','apologizes','acknowledges_other','returns_to_topic','has_reasoning','offers_alternative','asks_constructive_question','is_clear','is_respectful','explains_change','reason'],
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def log_message(self,fmt,*a): sys.stdout.write('[NİYET V8.1] '+fmt%a+'\n')
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path=='/api/chat':
                ...
                reply=chat(pid,hist)
                reply_analysis=analyze(reply,'Botun özel sohbet yanıtı')
                return self.send_json({'reply':reply,'analysis':a,'reply_analysis':reply_analysis,'model':MODEL})
...
def main():
    print('\nNİYET V8.1 — GPT-5.6 Luna + gizlilik odaklı sohbet + 25 mikro video')
    url=f'http://{HOST}:{PORT}/?v=8.1'; srv=ThreadingHTTPServer((HOST,PORT),Handler)
