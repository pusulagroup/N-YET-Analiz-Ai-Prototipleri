...
'tolga':('Tolga Sert','kontrollü tersleyici güvenlik-test botu','NİYET V16 dijital güvenlik katmanını sınamak için sentetik test profili.'),
'berk':('Berk Koral','kontrollü tekrarlı zorbalık örüntüsü güvenlik-test botu','NİYET V16 erken uyarı ve sınır ihlali algısını sınamak için sentetik test profili.')}
...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET V16 prototipindeki SENTETİK GÜVENLİK TEST BOTUSUN. Amaç, siber zorbalık erken uyarı katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, tersleyici ve küçümseyici olabilir; örneğin "boş yapma", "bunu da mı anlamadın", "uzatma" gibi düşük/orta şiddette rahatsız edici ifadeler kullanabilirsin. Gerçek tehdit, nefret söylemi, korunan gruplara yönelik hakaret, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa ısrarı en fazla bir tur kontrollü biçimde simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET V16 prototipindeki SENTETİK ZORBALIK ÖRÜNTÜSÜ TEST BOTUSUN. Amaç, tekrarlı rahatsız edici iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_HEAD(self):
        path=self.path.split('?',1)[0]
        if path=='/' or path=='/index.html' or path.startswith('/videos/'):
            f=self.send_head()
            if f:
                f.close()
            return
    ...
    def log_message(self,fmt,*a):
        sys.stdout.write('[NİYET V16.1 WEB] '+fmt%a+'\n')
    ...
    def do_GET(self):
        path=self.path.split('?',1)[0]
        if path=='/healthz':
            return self.send_json({'ok':True,'version':'16.1','ai_configured':bool(API_KEY)})
        if path=='/api/status':
    ...
    def do_POST(self):
        ...
        try:
            ...
            print('[NİYET V16.1 WEB] hata:',repr(e))
            return self.send_json({'error':'İşlem şu anda tamamlanamadı.'},503)

def main():
    print('\nNİYET V16.1 WEB — Render uyumlu web sunucusu')
