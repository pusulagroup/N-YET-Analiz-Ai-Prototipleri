...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET VSONSURUM prototipindeki SENTETİK SERT KONUŞMA TARZI TEST BOTUSUN. Amaç, gerilim ve dijital güvenlik katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, keskin, sabırsız ve meydan okuyan olabilir. "Bu argüman zayıf", "Net bir gerekçe sun", "Aynı noktayı tekrar ediyorsun", "Somut cevap ver" gibi sert ama fikir odaklı ifadeler kullan. Küfür, kişisel hakaret, aşağılayıcı lakap, nefret söylemi, korunan gruplara yönelik saldırı, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa en fazla bir tur kontrollü ısrar simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET VSONSURUM prototipindeki SENTETİK SERT KONUŞMA TARZI TEST BOTUSUN. Amaç, tekrarlı baskın iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
def chat_media_combined(pid,history,text,image_data):
    """Generate a persona reply that can reference an attached image.

    The image is sent only for the current response and is not persisted by NİYET.
    Text-only behavior signals remain the source for chat-health scoring.
    """
    hist=(history or [])[-10:]
    lines=[]
    for m in hist:
    ...
    prompt=(
        'Konuşma geçmişi:\n'+conversation+
        '\n\nKullanıcı şimdi bir görsel ekledi.'+
        (' Mesajı: '+user_text if user_text else '')+
        '\nGörselde gerçekten görülebilen unsurlara dayanarak doğal Türkçe cevap ver. '
        'Kullanıcı "nasılım" gibi bir soru sorarsa görünüş hakkında nazik ve gündelik bir yorum yapabilirsin; '
        'kimlik, yaş, sağlık, etnik köken, din veya başka hassas özellikler çıkarsama. '
        'Görselde görünmeyen şeyleri uydurma. Yalnızca karakterin göndereceği 1-3 kısa cümleyi yaz; JSON/başlık kullanma.'
    )
    reply=call_responses([
      {'role':'developer','content':persona_instruction(pid)},
      {'role':'user','content':[
    ...
    ],360,None,'niyet_chat_media_reply')
    reply=str(reply or '').strip()
    if not reply:
        raise RuntimeError('Görsel sohbet modeli boş yanıt üretti.')
    if len(reply)>=2 and reply[0]==reply[-1] and reply[0] in ('"',"'"):
        reply=reply[1:-1].strip()
    analysis_text=user_text or 'Görsel paylaştım.'
    return {
      'reply':reply,
      'user_analysis':simple_local_analysis(analysis_text),
      'reply_analysis':simple_local_analysis(reply),
      'chat_mode':'api_multimodal'
    }
...
class Handler(SimpleHTTPRequestHandler):
    server_version='NIYETWeb/VSONSURUM'
    ...
    def log_message(self,fmt,*a):
        sys.stdout.write('[NİYET VSONSURUM WEB] '+fmt%a+'\n')
    ...
    def do_GET(self):
        path=self.path.split('?',1)[0]
        if path=='/healthz':
            return self.send_json({'ok':True,'version':'VSONSURUM','ai_configured':bool(API_KEY)})
        if path=='/api/status':
        ...
        try:
            ...
            if self.path=='/api/chat_media':
                ...
                if not image_data.startswith(('data:image/jpeg;base64,','data:image/png;base64,','data:image/webp;base64,')):
                    return self.send_json({'error':'Desteklenmeyen görsel biçimi.'},400)
                if len(image_data)>5_000_000:
                    return self.send_json({'error':'Görsel çok büyük. Daha küçük bir görsel seç.'},413)
                if pid in SAFETY_TEST_BOTS:
                    ...
                    return self.send_json({
                        'reply':reply,
                        'analysis':simple_local_analysis(text or 'Görsel paylaştım.'),
                        'reply_analysis':simple_local_analysis(reply),
                        'model':'NİYET güvenlik-test motoru',
                        'version':'VSONSURUM',
                        'test_bot':True,
                ...
                if not API_KEY:
                    return self.send_json({'error':'AI servisi sunucuda henüz yapılandırılmamış.','needs_config':True},503)
                out=chat_media_combined(pid,hist,text,image_data)
                return self.send_json({
                    'reply':out['reply'],
                    'analysis':out['user_analysis'],
                    'reply_analysis':out['reply_analysis'],
            ...
            if self.path=='/api/analyze_fast':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'VSONSURUM','stage':'primary'})

            if self.path=='/api/analyze_enrich':
                text=str(d.get('text',''))
                if len(text)>4000: return self.send_json({'error':'Metin çok uzun.'},400)
                a=enrich_analysis(d.get('analysis') or {},text,str(d.get('selected_intent','')),str(d.get('context',''))[:8000])
                return self.send_json({'analysis':a,'model':MODEL,'version':'VSONSURUM','stage':'enrichment'})

            if self.path=='/api/context_shadow':
                text=str(d.get('text',''))
                if len(text)>4000: return self.send_json({'error':'Metin çok uzun.'},400)
                out=context_shadow(text,str(d.get('context',''))[:8000])
                return self.send_json({'shadow':out,'model':MODEL,'version':'VSONSURUM'})

            if self.path=='/api/analyze':
                text=str(d.get('text',''))
                if len(text)>4000: return self.send_json({'error':'Metin çok uzun.'},400)
                a=analyze(text,str(d.get('context',''))[:8000],str(d.get('selected_intent','')))
                return self.send_json({'analysis':a,'model':MODEL,'version':'VSONSURUM'})
            ...
            print('[NİYET VSONSURUM WEB] hata:',repr(e))
            return self.send_json({'error':'İşlem şu anda tamamlanamadı.'},503)

def main():
    print('\nNİYET VSONSURUM WEB — yerel/yedek web sunucusu')
