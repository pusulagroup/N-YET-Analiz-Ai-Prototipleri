...
import json, os, sys, urllib.request, urllib.error, webbrowser, threading, math, re, difflib, concurrent.futures, copy, hashlib

ROOT=Path(__file__).resolve().parent
HOST='127.0.0.1'
PORT=int(os.getenv('NIYET_PORT','8852'))
...
'tolga':('Tolga Sert','kontrollü tersleyici güvenlik-test botu','NİYET V14 dijital güvenlik katmanını sınamak için sentetik test profili.'),
'berk':('Berk Koral','kontrollü tekrarlı zorbalık örüntüsü güvenlik-test botu','NİYET V14 erken uyarı ve sınır ihlali algısını sınamak için sentetik test profili.')}
...
CONTEXT_SHADOW_SCHEMA={
  ...
  'required':['standalone_distribution','contextual_distribution','standalone_reason','contextual_reason'],
  'additionalProperties':False
}
...
def load_calibration():
    ...
    try:
        ...
        print('[NİYET V14] calibration_profile.json okunamadı:',e)
...
def evaluate_variants(items,context=''):
    ...
    checks run in parallel where possible so Algı Kararlılığı / Minimum Müdahale
    ...
    def eval_chunk(chunk, pos):
        ...
        try:
            ...
            print(f'[NİYET V14] varyant batch uyarısı ({pos}): {batch_error}')
        for item in chunk:
            try:
                d=call_responses(batch_perception_prompt([item],context),620,BATCH_SCHEMA,'niyet_v14_variant_single')
                consume(d)
            except Exception as single_error:
                print(f"[NİYET V14] varyant atlandı {item.get('id','?')}: {single_error}")
        return local
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(3,len(chunks))) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            except Exception as e: print('[NİYET V14] paralel varyant uyarısı:',e)
    return out
...
def attach_v13_analysis(original,text,selected_intent,context=''):
    ...
    if selected_intent in INTENTS:
        try:
            ...
            print(f'[NİYET V14] Minimum Müdahale aday üretimi uyarısı: {e}')
...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET V14 prototipindeki SENTETİK GÜVENLİK TEST BOTUSUN. Amaç, siber zorbalık erken uyarı katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, tersleyici ve küçümseyici olabilir; örneğin "boş yapma", "bunu da mı anlamadın", "uzatma" gibi düşük/orta şiddette rahatsız edici ifadeler kullanabilirsin. Gerçek tehdit, nefret söylemi, korunan gruplara yönelik hakaret, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa ısrarı en fazla bir tur kontrollü biçimde simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET V14 prototipindeki SENTETİK ZORBALIK ÖRÜNTÜSÜ TEST BOTUSUN. Amaç, tekrarlı rahatsız edici iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
def analyze_fast(text,context='',selected_intent=''):
    key=_cache_key('fast',MODEL,text,context,selected_intent,CALIBRATION.get('version'))
    cached=_cache_get(_FAST_CACHE,key)
    if cached is not None:
        cached['cache_hit']=True
        return cached
    a=call_responses(fast_analysis_prompt(text,context),900,core_schema(),'niyet_v14_fast_analysis')
...
def enrich_analysis(original,text,selected_intent,context=''):
    ...
    try:
        ...
        print(f'[NİYET V14] zenginleştirme uyarısı: {e}')
        base['stability']={'available':False,'score':None,'label':'Bu analizde hesaplanamadı','variant_count':0}
        base['interventions']=[]
        base.setdefault('enrichment_warnings',[]).append('Algı Kararlılığı/Minimum Müdahale ek hesaplarından biri tamamlanamadı; ana insan-kalibre algı sonucu korunmuştur.')
        out=base
...
def context_shadow(text,context=''):
    ...
    if cached is not None:
        cached['cache_hit']=True
        return cached
    d=call_responses(context_shadow_prompt(text,context),760,CONTEXT_SHADOW_SCHEMA,'niyet_v14_context_shadow')
    ...
    # Prototype categorical thresholds; intentionally no extra 0-100 user score.
    if distance>=0.28 or (sdom!=cdom and distance>=0.18): level='Yüksek'
    elif distance>=0.12 or sdom!=cdom: level='Orta'
    else: level='Düşük'
    unc=perception_uncertainty(contextual)
    ranked=sorted(contextual.items(),key=lambda kv:kv[1],reverse=True)
    warning=None
    if unc>=65 and len(ranked)>=2:
...
def chat_combined(pid,history):
    ...
    try:
        ...
        print(f'[NİYET V14] sohbet yedek yanıtı ({pid}): {e}')
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def log_message(self,fmt,*a): sys.stdout.write('[NİYET V14] '+fmt%a+'\n')
    ...
    def do_GET(self):
        if self.path=='/api/status':
            return self.send_json({'configured':bool(API_KEY),'model':MODEL,'version':'14.0','calibration':calibration_meta()})
        ...
        try:
            ...
            if self.path=='/api/chat':
                ...
                if pid in SAFETY_TEST_BOTS:
                    ...
                    return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':'V14 güvenlik-test motoru','version':'14.0','test_bot':True})
            ...
            if self.path=='/api/analyze_fast':
                a=analyze_fast(str(d.get('text','')),str(d.get('context','')),str(d.get('selected_intent','')))
                return self.send_json({'analysis':a,'model':MODEL,'version':'14.0','stage':'primary'})
            if self.path=='/api/analyze_enrich':
                a=enrich_analysis(d.get('analysis') or {},str(d.get('text','')),str(d.get('selected_intent','')),str(d.get('context','')))
                return self.send_json({'analysis':a,'model':MODEL,'version':'14.0','stage':'enrichment'})
            if self.path=='/api/context_shadow':
                out=context_shadow(str(d.get('text','')),str(d.get('context','')))
                return self.send_json({'shadow':out,'model':MODEL,'version':'14.0'})
            if self.path=='/api/analyze':
                a=analyze(str(d.get('text','')),str(d.get('context','')),str(d.get('selected_intent','')))
                return self.send_json({'analysis':a,'model':MODEL,'version':'14.0'})
            if self.path=='/api/chat':
                pid=str(d.get('person_id','')); hist=d.get('history') or []; opening=bool(d.get('opening',False))
                ...
                model_label=MODEL if out.get('chat_mode')!='local_fallback' else 'V14 yerel sohbet yedeği'
                return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':model_label,'version':'14.0','chat_mode':out.get('chat_mode')})
            if self.path=='/api/bot_comment':
                ...
                return self.send_json({'reply':out['reply'],'user_analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'14.0'})
            return self.send_json({'error':'Not found'},404)
        except Exception as e:
            return self.send_json({'error':str(e)},503)

def main():
    print('\nNİYET V14 — Hızlı iki aşamalı analiz + Gölge Niyet + Genç Dijital Kalkanı')
    preferred=PORT
    ...
    if srv is None:
        ...
        raise SystemExit(1)
    url=f'http://{HOST}:{active_port}/?v=14.0'
