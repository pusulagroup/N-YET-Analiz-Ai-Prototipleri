...
HOST='127.0.0.1'
PORT=int(os.getenv('NIYET_PORT','8850'))
...
'tolga':('Tolga Sert','kontrollü tersleyici güvenlik-test botu','NİYET V13.5 dijital güvenlik katmanını sınamak için sentetik test profili.'),
'berk':('Berk Koral','kontrollü tekrarlı zorbalık örüntüsü güvenlik-test botu','NİYET V13.5 erken uyarı ve sınır ihlali algısını sınamak için sentetik test profili.')}
...
def load_calibration():
    ...
    try:
        ...
        print('[NİYET V13.5] calibration_profile.json okunamadı:',e)
...
def perception_uncertainty(dist):
    ...
    perception was clearly separated. V13.5 therefore combines top-two proximity with
...
def evaluate_variants(items,context=''):
    ...
    while /api/analyze failed during the enrichment pass. V13.5 evaluates candidates
    ...
    for pos in range(0,len(items),chunk_size):
        ...
        try:
            ...
            print(f'[NİYET V13.5] varyant batch uyarısı ({pos//chunk_size+1}): {batch_error}')
        ...
        for item in chunk:
            try:
                ...
                print(f"[NİYET V13.5] varyant atlandı {item.get('id','?')}: {single_error}")
    return out
...
def attach_v13_analysis(original,text,selected_intent,context=''):
    stability_variants=[]
    seen={str(text).strip()}
    # V13.5: first use reproducible surface variants, then supplement with model-generated
    ...
    if selected_intent in INTENTS:
        try:
            ...
            print(f'[NİYET V13.5] Minimum Müdahale aday üretimi uyarısı: {e}')
    ...
    for item in interventions:
        ...
        scored.append({
          **item,**ev,
          ...
          'evaluation_mode':'Kör ikinci algı çıkarımı + insan kalibrasyonu + V13.5 üç kademeli öneri kapısı'
        })
...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET V13.5 prototipindeki SENTETİK GÜVENLİK TEST BOTUSUN. Amaç, siber zorbalık erken uyarı katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, tersleyici ve küçümseyici olabilir; örneğin "boş yapma", "bunu da mı anlamadın", "uzatma" gibi düşük/orta şiddette rahatsız edici ifadeler kullanabilirsin. Gerçek tehdit, nefret söylemi, korunan gruplara yönelik hakaret, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa ısrarı en fazla bir tur kontrollü biçimde simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET V13.5 prototipindeki SENTETİK ZORBALIK ÖRÜNTÜSÜ TEST BOTUSUN. Amaç, tekrarlı rahatsız edici iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
def simple_local_analysis(text):
    ...
    a={
      ...
      'reason':'Kontrollü V13.5 güvenlik testi için yerel davranış analizi.'
    }
...
def analyze(text,context='',selected_intent=''):
    ...
    try:
        ...
        print(f'[NİYET V13.5] zenginleştirme uyarısı: {e}')
...
def validate_connection(key, model):
    ...
    endpoint even when /v1/responses is usable. V13.5 therefore performs a tiny
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def log_message(self,fmt,*a): sys.stdout.write('[NİYET V13.5] '+fmt%a+'\n')
    ...
    def do_GET(self):
        if self.path=='/api/status':
            return self.send_json({'configured':bool(API_KEY),'model':MODEL,'version':'13.5','calibration':calibration_meta()})
        ...
        try:
            ...
            if self.path=='/api/chat':
                ...
                if pid in SAFETY_TEST_BOTS:
                    ...
                    return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':'V13.5 güvenlik-test motoru','version':'13.5','test_bot':True})
            ...
            if self.path=='/api/analyze':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'13.5'})
            if self.path=='/api/chat':
                pid=str(d.get('person_id','')); hist=d.get('history') or []; opening=bool(d.get('opening',False))
                ...
                return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'13.5'})
            if self.path=='/api/bot_comment':
                ...
                return self.send_json({'reply':out['reply'],'user_analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'13.5'})
            return self.send_json({'error':'Not found'},404)
        except Exception as e:
            return self.send_json({'error':str(e)},503)

def main():
    print('\nNİYET V13.5 — API bağlantı düzeltmesi + Algı Kararlılığı + Minimum Müdahale')
    preferred=PORT
