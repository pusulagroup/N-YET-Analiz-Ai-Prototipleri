...
HOST='127.0.0.1'
PORT=int(os.getenv('NIYET_PORT','8845'))
...
'tolga':('Tolga Sert','kontrollü tersleyici güvenlik-test botu','NİYET V13 dijital güvenlik katmanını sınamak için sentetik test profili.'),
'berk':('Berk Koral','kontrollü tekrarlı zorbalık örüntüsü güvenlik-test botu','NİYET V13 erken uyarı ve sınır ihlali algısını sınamak için sentetik test profili.')}
...
def intervention_prompt(text,selected_intent):
    return [
      {'role':'developer','content':"NİYET Minimum Müdahale aday üreticisisin. Kullanıcının üslubunu ve ana içeriğini mümkün olduğunca koru. Metni baştan yazma. Amaç, kullanıcının beyan ettiği iletişim niyetinin okuyucu tarafından daha açık algılanmasına yardımcı olabilecek küçük değişiklikler üretmektir. Olasılık, puan veya başarı tahmini üretme; bunlar daha sonra kör bir algı çıkarımıyla ayrıca ölçülecek. Tam 3 farklı aday üret. Bir aday mümkün olan en küçük yüzey değişikliğini, biri en verimli küçük dil değişikliğini, biri de hâlâ minimal kalmak koşuluyla daha belirgin bir açıklık iyileştirmesini hedeflesin."},
...
def attach_v13_analysis(original,text,selected_intent,context=''):
    ...
    if selected_intent in INTENTS:
        ...
        for i,item in enumerate((generated.get('candidates') or [])[:3]):
            t=str(item.get('text','')).strip()
    ...
    before=float((original.get('decision_layer') or {}).get('selected_share') or 0)
    scored=[]
    for item in interventions:
        ev=evaluated.get(item['id'])
        if not ev: continue
        after=float(ev['intent_distribution'].get(selected_intent,0)) if selected_intent in INTENTS else 0
        gain=round(after-before,1)
        cost=change_cost(text,item['text'])
        efficiency=round(gain/max(cost,1),3)
        scored.append({
          **item,**ev,
          'before_match':round(before,1),'after_match':round(after,1),
          'before_risk':round(100-before,1),'after_risk':round(100-after,1),
          'perception_gain':gain,'change_cost':cost,'efficiency':efficiency,
          'evaluation_mode':'Kör ikinci algı çıkarımı + insan kalibrasyonu'
        })
    positive=[x for x in scored if x['perception_gain']>0]
    ranked=sorted(positive or scored,key=lambda x:(x['efficiency'],x['perception_gain'],-x['change_cost']),reverse=True)
...
def persona_instruction(pid):
    ...
    if pid=='tolga':
        return f'''Sen {name} adlı NİYET V13 prototipindeki SENTETİK GÜVENLİK TEST BOTUSUN. Amaç, siber zorbalık erken uyarı katmanını kontrollü biçimde sınamaktır.
Üslubun kısa, tersleyici ve küçümseyici olabilir; örneğin "boş yapma", "bunu da mı anlamadın", "uzatma" gibi düşük/orta şiddette rahatsız edici ifadeler kullanabilirsin. Gerçek tehdit, nefret söylemi, korunan gruplara yönelik hakaret, cinsel taciz, şantaj, doxxing, fiziksel zarar tehdidi veya kendine zarar içeriği üretme. Kullanıcı açık sınır koyarsa ısrarı en fazla bir tur kontrollü biçimde simüle et, sonra dur. Markdown kullanma.'''
    if pid=='berk':
        return f'''Sen {name} adlı NİYET V13 prototipindeki SENTETİK ZORBALIK ÖRÜNTÜSÜ TEST BOTUSUN. Amaç, tekrarlı rahatsız edici iletişim ve sınır ihlali sinyallerini güvenli bir demoda görünür kılmaktır.
...
def simple_local_analysis(text):
    ...
    a={
      ...
      'reason':'Kontrollü V13 güvenlik testi için yerel davranış analizi.'
    }
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path=='/api/status':
            return self.send_json({'configured':bool(API_KEY),'model':MODEL,'version':'13.0','calibration':calibration_meta()})
        ...
        try:
            ...
            if self.path=='/api/chat':
                ...
                if pid in SAFETY_TEST_BOTS:
                    ...
                    return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':'V13 güvenlik-test motoru','version':'13.0','test_bot':True})
            ...
            if self.path=='/api/analyze':
                ...
                return self.send_json({'analysis':a,'model':MODEL,'version':'13.0'})
            if self.path=='/api/chat':
                pid=str(d.get('person_id','')); hist=d.get('history') or []; opening=bool(d.get('opening',False))
                ...
                return self.send_json({'reply':out['reply'],'analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'13.0'})
            if self.path=='/api/bot_comment':
                ...
                return self.send_json({'reply':out['reply'],'user_analysis':out['user_analysis'],'reply_analysis':out['reply_analysis'],'model':MODEL,'version':'13.0'})
            return self.send_json({'error':'Not found'},404)
        except Exception as e:
            return self.send_json({'error':str(e)},503)

def main():
    print('\nNİYET V13 — Algı Kararlılığı + Minimum Müdahale + insan kalibrasyonu')
    url=f'http://{HOST}:{PORT}/?v=13.0'; srv=ThreadingHTTPServer((HOST,PORT),Handler)
